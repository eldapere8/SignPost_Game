...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        int count =0;
        int sCount=0;
        for (int row =0; row < model.width(); row+=1){
            for (int col =0; col< model.height(); col+=1){
                if (start.connectable(model.get(row,col))) count+=1;
            }
        }
        if (count==1) {
            for (Sq potentialReturn : model) {
                if (start.connectable(potentialReturn)) return potentialReturn;
            }
        }

        if (start.sequenceNum()!=0) {
            for (Sq potentialReturn : model) {if (start.connectable(potentialReturn) && (start.sequenceNum() == potentialReturn.sequenceNum()-1)) return potentialReturn;
            }
        }
        return null;
    }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        ...
        for (int row =0; row < model.width(); row+=1){
            for (int col =0; col< model.height(); col+=1){
                if (model.get(row,col).connectable(end)) count+=1;
            }
        }
        if (count==1) {
            for (Sq potentialReturn : model) {
                if (potentialReturn.connectable(end)) return potentialReturn;
            }
        }
